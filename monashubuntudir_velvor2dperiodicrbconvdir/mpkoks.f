...
!mpkrb       randno=0.5d0+0.5d0*sin(dble(k)/150.d0*2.d0*pi)
           randno=0.5d0+0.5d0*sin(dble(k)/dble(np))
           th(i,j,k)=th(i,j,k)+(r-r0)/rmax*0.01*((randno*2.d0)-1.d0)
         endif
  10  continue
!mpk      do 11 j=1,nz-1
...
!mpkrb        httrans(ntot+kt)=-r0*dlog(r0/rmax)*trans1/(2.d0*pi)
        httrans(ntot+kt)=trans1*(rmax-r0)/xx
!mpkrb        httrans2(ntot+kt)=-rmax*dlog(r0/rmax)*trans2/(2.d0*pi)
        httrans2(ntot+kt)=trans2*(rmax-r0)/xx
...
c        do i=1,nr
c         do k=0,np
c          thn(i,j,k)=thn(i,j,k)-Ap(i,j,k)
c         enddo
c        enddo
