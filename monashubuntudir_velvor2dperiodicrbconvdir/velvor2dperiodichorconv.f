...
!mpk2d     +             *th(i,j,k)
         ...
         elseif(i.eq.nr+1)then
          if(k.lt.np/2)then
           th(nr+1,j,k)=dble(k)/(0.5d0*dble(np))*1.d0
!mpkhc           th(nr+1,j,k)=1.d0
          elseif(k.ge.np/2)then
           th(nr+1,j,k)=dble(np-k)/(0.5d0*dble(np))*1.d0
!mpkhc           th(nr+1,j,k)=1.d0
          endif
!mpkhc           th(nr+1,j,k)=1.d0
...
!mpk2d------------------ compact FD, finding thh
 ...
 !mpkbug?    +                th(i,j,k))*dpi
           tdmar(k+1)=3.d0*(th(i+1,j,ka)+th(i,j,ka)-th(i+1,j,k)-
     +                th(i,j,k))*dpi
       enddo
       call ptdma(tdmaa,tdmab,tdmac,tdmas,tdmar,np+1)
       do kk=0,np
...
!mpkhc end
        if(k.lt.np/2)then
           thn(nr+1,j,k)=dble(k)/(0.5d0*dble(np))*1.d0
!mpkhc           thn(nr+1,j,k)=1.d0
          elseif(k.ge.np/2)then
           thn(nr+1,j,k)=dble(np-k)/(0.5d0*dble(np))*1.d0
!mpkhc           thn(nr+1,j,k)=1.d0
          endif
!mpkhc        thn(nr+1,j,k)=1.d0
  30  continue
