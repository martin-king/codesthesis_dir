...
c     free convection in stationary horizontal annulus.                c
c     mpk.retains original non-d scalings by Lewis.                    c
c        .introduces axial gravitational terms(present only in Avort   c
c         and Bvort. A Grashof number(Gr) entails. Gr loaned to g      c                   
c         parallel to r-theta plane. this term will appear in all      c                                    
c         components of vorticity.                                     c
c        .hybrid scheme introduced in ABCVort and Temp(commented out)  c
c        .corrections                                                  c
c        .u, v, w located at walls on bound                            c
c        .Coriolis force deleted                                       c 
c        .T_i=1.d0,T_o=0.d0                                            c
c         kr=65,kz=65,kp=128,mgen=626953                               c
c       .extrapolation and SOR added in subroutine POISSON?            c
c        intersweeps SOR(interant recombination)                       c     
c         LAST UPDATED: 28-2-01.                                      c                                       
c----------------------------------------------------------------------c
c
      ...
      real*8 httrans1(0:mgen),httrans2(0:mgen)
...
!mpk  11  continue
      th(nr/2,nz/2,np/2)=th(nr/2,nz/2,np/2)-0.5d0*th(nr/2,nz/2,np/2)
c
!mpk      do 12 i=1,nr
!mpk      do 12 j=1,nz
!mpk      do 12 k=0,np
...
!mpk        call aaaaaaaa(ntot+kt)
        ...
        call update
c     cal Nu along 3 cells at mid-axis, on inner & outer cylinders
        httrans1(ntot+kt)=dble(nz)/6.d0*r0*dlog(r0/rmax)*trans1/
     +         (zmax*2.d0*pi)
        httrans2(ntot+kt)=dble(nz)/6.d0*rmax*dlog(r0/rmax)*trans2/
     +         (zmax*2.d0*pi)
        print*,httrans1(ntot+kt),httrans2(ntot+kt),ntot+kt
c
c        call rufcon(th,nr+1,nz+1,np)
  20  continue
c
      ntot=ntot+nts
c
...
c
      if(fname8 .ne. 'no')then
        ...
        write(11,200)((i-1)*dt,httrans1(i),httrans2(i),i=1,ntot)
...
c
 200  format(3(1x,e16.8))
!mpk 200  format(14(1x,e16.8))
c
      stop
      end
c
...
!mpk        RHS2=-2.d0*Re*uelz+2.d0*Ra*(tem*uelz+thz*u)/Re/Ptl
!mpk     +       -Gr*Ptl/r*thh
        RHS2=sintheta*Gr*Ptl*thz
        cf1=1.d0/(Ptl*dt)-uelr/(2.d0*Ptl)+1.d0/(2.d0*r**2)-cnr-cnz-cnh
        cf2=uelr/(2.d0*Ptl)-1.d0/(2.d0*r**2)+cnr+cnz+cnh
!mpk        An(i,j,k)=(cf2*Ap(i,j,k)-ULT+RHS1)/cf1
...
!mpk        RHS2=-2.d0*Re*velz+r*Ra*thz+2.d0*Ra*(tem*velz+thz*v)/Re/Ptl
!mpk     +       +Gr*Ptl*thr
        RHS2=costheta*Gr*Ptl*thz
...
!mpk        Bn(i,j,k)=(cf2*Bp(i,j,k)-ULT+RHS1)/cf1
c
  10  continue
c
      return
...
!mpk        RHS2=-2.d0*Re*welz-Ra*thh+2.d0*Ra*(tem*welz-thr*u-thh*v/r)
!mpk     +       /Re/Ptl
        RHS2=-1.d0/r*Gr*Ptl*costheta*thh-Gr*Ptl*sintheta*thr
        cf1=1.d0/(Ptl*dt)-welz/(2.d0*Ptl)-cnr-cnz-cnh
        cf2=welz/(2.d0*Ptl)+cnr+cnz+cnh
        Cn(i,j,k)=(cf2*Cp(i,j,k)-ULT-RHS2+RHS1)/cf1
!mpk        Cn(i,j,k)=(cf2*Cp(i,j,k)-ULT+RHS1)/cf1
c
  10  continue
c
      return
...
c     on the inner and outer cylinder near mid-axis                    c
c----------------------------------------------------------------------c
c
...
c
      ...
      nzb=nz/2+3
      do 10 k=1,np
      transa=dz*(-184.d0*th(0,nza,k)+225.d0*th(1,nza,k)
     +     -50.d0*th(2,nza,k)+9.d0*th(3,nza,k))/(120.d0*dr)
      transb=dz*(184.d0*th(nr+1,nza,k)-225.d0*th(nr,nza,k)+50.d0
     +     *th(nr-1,nza,k)-9.d0*th(nr-2,nza,k))/(120.d0*dr)
      transa=transa+dz*(-184.d0*th(0,nzb,k)+225.d0*th(1,nzb,k)
     +     -50.d0*th(2,nzb,k)+9.d0*th(3,nzb,k))/(120.d0*dr)
      transb=transb+dz*(184.d0*th(nr+1,nzb,k)-225.d0*th(nr,nzb,k)+50.d0
     +     *th(nr-1,nzb,k)-9.d0*th(nr-2,nzb,k))/(120.d0*dr)
c
      do 20 j=nza+1,nzb-1
        transa=transa+dz*(-184.d0*th(0,j,k)+225.d0*th(1,j,k)-50.d0
     +       *th(2,j,k)+9.d0*th(3,j,k))/(60.d0*dr)
        transb=transb+dz*(184.d0*th(nr+1,j,k)-225.d0*th(nr,j,k)+50.d0
     +     *th(nr-1,j,k)-9.d0*th(nr-2,j,k))/(60.d0*dr)
  20  continue
        trans1=trans1+transa*dp
        trans2=trans2+transb*dp
  10  continue
c
      return
      end
      ...
      do 10 n=1,6
        ...
        do 20 k=1,np
          ...
          do 30 j=0,nz+1
            ...
            if(j.eq.0.or.j.eq.nz+1)fact=fact/2.d0
            fc=r*th(i,j,k)*dcos(dble(n)*ang)
            fs=r*th(i,j,k)*dsin(dble(n)*ang)
            sumc=sumc+fc*fact
            sums=sums+fs*fact
   30     continue
          sumc=sumc*dr*dz
          sums=sums*dr*dz
c
          ssumc(n,kt)=ssumc(n,kt)+sumc*dp
          ssums(n,kt)=ssums(n,kt)+sums*dp
   20   continue
   10 continue
c
      return
...
!mpk        do 31 js=0,nz
!mpk        do 31 ks=0,np
!mpk        psif(is,js,ks)=psip(is,js,ks)+1.2d0*(psif(is,js,ks)-
!mpk     +    psip(is,js,ks))        
!mpk        psip(is,js,ks)=psif(is,js,ks)
!mpk   31   continue
!mpk        call trans1(psif,v,ibeg(1),n1(1),n2(1),n3(1))
!mpk        endif
...
!mpk            do 55 jj=0,nz
!mpk            do 55 kk=0,np
!mpk            psip(ii,jj,kk)=psif(ii,jj,kk)
!mpk   55     continue
        end if
        if(nlev.gt.1)then
          do 51 ilev=1,nlev-1
...
c
      ...
      do 10 i=0,n1
        ...
        if(ii.gt.202590)then
         print*,'FATAL ERROR IN MULTIGRID  ii='
         STOP
        endif        
        a1d(ii)=a3d(i,j,k)
   10 continue
