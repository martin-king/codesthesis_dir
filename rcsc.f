...
c     Program rcsc.f                                                   c
c     ==============                                                   c
c     T.Lewis's rot7.f modified by martin king, 18 Nov 1999.           c                                               
c     Mathematical modelling of flow in a rotating annulus             c
c     in r-z plane. rotating cavity with stationary casing             c
...
c     with new rotation dominated scaling                              c
c----------------------------------------------------------------------c
c
      implicit real*8(a-h,o-z)
      parameter(nrmax=200,nzmax=200,mgen=100000,na=8)
...
!mpk      read*,fname4
     ...
     +    velocities & standard deviations?(0 for no): '
      read*,nff
      print*,'filename for max/min vorticity data at every timestep
     +          (filename or "no"): '
      read*,fname5
      print*,'filename for max/min stream function at every timestep
     +          (filename or "no"): ' 
      read*,fname6
      print*,'time step: '
      read*,dt
...
c        vel(n1(1),j)=-Ptl*Re*r(i,1)
        vel(i,j)=-r(i,1)   ! start with 0 rotation(rot frame)
...
!mpk    writing statistical at nx_3 & nx_5,at z/s=0.8,at evry tmstp
        nx_5=n1(1)/2*0.5
!mpk        nx_3=n1(1)/2*0.3
!mpk        PRINT*,mean_vel(nx_3,3),mean_vel(nx_5,3)
        WRITE(99,51)k*dt,mean_vel(nx_5,3) !mpk ,mean_vel(nx_3,3)
...
c
        Pe=dabs(0.5d0*phiz*(hr(i,1)+hr(i+1,1))*Re)                
        if(Pe.gt.2)then
        if(phiz.gt.0)then
...
c
        Pe=dabs(0.5d0*(-phir-phi(i,j)/r(i,1))*(hz(j,1)+hz(j+1,1))*Re) 
        if(Pe.gt.2)then
...
c       
        RHS1=omegrra+omegra/r(i,1)+omegzza+omeg(i,j)/dt*Re
        ULT=((phiz*omegr-phir*omegz)-(phi(i,j)*omegz)/r(i,1))*Re
        RHS2=-2.d0*Re*vel(i,j)*velz/r(i,1)-2.d0*Re*velz
!mpk     +      +2.d0*Ra*(theta(i,j)*velz+thetaz*vel(i,j))/(Re*Ptl)
!mpk     +      +r(i,1)*Ra*thetaz
        cf1=1.d0/dt*Re+1.d0/(2.d0*r(i,1)**2)-cnr-cnz-phiz/(2.d0
     +     *r(i,1))*Re
        cf2=-1.d0/(2.d0*r(i,1)**2)+cnr+cnz+phiz/(2.d0*r(i,1))*Re
c
        omegn(i,j)=(cf2*omegp(i,j)-ULT-RHS2+RHS1)/cf1
c        print*,'omegn=',omegn(i,j)
c
  10  continue      
c
      return
...
c
        RHS1=velrra+velra/r(i,1)+velzza+vel(i,j)/dt*Re
        ULT=((phiz*velr-phir*velz)-(phi(i,j)*velz)/r(i,1))*Re
        RHS2=2.d0*Re*phiz !mpk -2.d0*Ra*theta(i,j)*phiz/(Re*Ptl)
        cf1=1.d0/dt*Re+1.d0/(2.d0*r(i,1)**2)-cnr-cnz
     +     +phiz/(2.d0*r(i,1))*Re
        cf2=-1.d0/(2.d0*r(i,1)**2)+cnr+cnz-phiz/(2.d0*r(i,1))*Re
c
        veln(i,j)=(cf2*velp(i,j)-ULT-RHS2+RHS1)/cf1
c        print*,'veln=',veln(i,j)
c
  10  continue
c
        return
...
c
      implicit real*8(a-h,o-z)
      parameter (nrmax=200,nzmax=200,na=8)
      parameter (vis=1.82d-5,denref=1.19d0)
      real*8 hr(nrmax,na),hz(nzmax,na),r(0:nrmax,na),z(0:nzmax,na)
      real*8 omegn(0:nrmax,0:nzmax),omeg(0:nrmax,0:nzmax)
      real*8 omegp(0:nrmax,0:nzmax),phi(0:nrmax,0:nzmax)
      real*8 veln(0:nrmax,0:nzmax),vel(0:nrmax,0:nzmax)
...
c      
      ...
      r_d(0)=0.382d0*r(0,1)
      z_d(0)=0.382d0*z(0,1)
      omref_d=Re*vis/denref/rmax_d**2
c
      do 3 i=1,n1(1)
...
c
      do 6 i=0,n1(1)
      do 7 j=0,n2(1)
      vel_d(i,j)=(vel(i,j)*rmax_d*omref_d)
    7 continue
    6 continue   
c     
c-------------------------------------------------------------------
...
c     1  +r*vel_d(2,j)/r_d(2))
      ...
      ssw=(sswp+sswn)/2d0
      sswp=sswn
c      s_temp=ssw*2*pi*r0_d**2*hz_d(j)*2/denref/omref_d**2/rmax_d**5
c      print*,z_d(j),s_temp
...
c     2  +r*vel_d(n1(1),j)/r_d(n1(1))
      ...
      ssw=(sswp+sswn)/2d0
      sswp=sswn
c      s_temp=ssw*2*pi*rmax_d**2*hz_d(j)*2/denref/omref_d**2/rmax_d**5
c      print*,z_d(j),s_temp
...
c      sswn=vis*(p*vel_d(i,0)+q*vel_d(i,1)+r*vel_d(i,2))
      ...
      ssw=(sswp+sswn)/2d0
      sswp=sswn
c      print*,r_d(i),ssw*2*pi*hr_d(i)*2/denref/omref_d**2/rmax_d**5
...
!by all means, avoid numerical differentiation. when cal the mean.
      b=2*(a-ap)/dt*omref_d/omref_d**2/rmax_d**5   
!mpk
...
c      
     ...
     2   ABS(cmr_mean(2)))
      I_mean=(a-a_1st)/(nts*dt)*2*omref_d/omref_d**2/rmax_d**5 
...
c    ------------------------------------------------------------------c
      implicit real*8(a-h,o-z)
      parameter(kr=200,kz=200,mgen=100000,na=8)
...
c==================================================
      ...
      implicit real*8(a-h,o-z)
      parameter(kr=200,kz=200,mgen=100000)
      real*8 a2d(0:kr,0:kz),a1d(mgen)
c
      do 10 j=0,n2
      do 10 i=0,n1
        ii=j*(n1+1) + i + i1
c        if (ii.gt.mgen)then 
c         print*,'mgen Fatal Error...Program Terminated!'
c         STOP
c        endif
        a1d(ii)=a2d(i,j)
   10 continue
...
c==================================================
      ...
      parameter(mx=200,my=200,mgen=100000)
      implicit real*8(a-h,o-z)
      real*8 a(0:mx,0:my)
      real*8 b(0:mx,0:my)
      character*1 c(0:401)
      ...
      nb=xb
      c(i)=ch(na)
      if(i+nx+2.gt.401)print*,'ERROR IN RUFCON2!'
      c(i+nx+2)=ch(nb)
  10  continue
      m2=nx+1
      m1=0
