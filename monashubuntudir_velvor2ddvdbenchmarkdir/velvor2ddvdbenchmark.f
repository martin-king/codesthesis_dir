...
c     Program velvor2ddvdbenchmark.f                                 c
c======================================================================c
c     descendent of a code by T.Lewis(1999).                           c
c     Last modified by Martin King 7 Mar 2008.                         c
c     mpk  to simulate R-B convection in a 2D periodic box of          c
c     mpk  2x0.5 (widthxheight)                                        c
c     Three dimensional velocity/vorticity model on staggered grid.    c
c     (mpk: originally, see modification records below)                c
cmpk===================modification history============================c
...
c        .MAJOR for this version 7 Mar 2008: modified to do RB conv in c
c         a periodic 2D Box;  removed terms containing Re, etc.        c  
c         Commented with !mpkrb                                        c
c        .MAJOR 14 Oct 2008: changed b.c's to do horizontal convection c
c         and other necessary related changes. !mpkhc                  c
c        .MAJOR 5 Feb 2009: changed b.c.'s to do a tank (instead of    c
c         periodic side boundaries. insulated side & top walls         c
c        !mpktank                                                      c
c         needed to add np+1 for temperature, can search for 'np+1'    c
c        .MAJOR 18 Feb 2009: so now i can have a sealed cavity like in c
c         De Vahl Davis. I set T=1 at i=nr+1, T=0 at i=0 and use       c
c         RHS2=-Ra*thr in cvorticity !mpkdvd                           c
c                                                                      c
c----------------------------------------------------------------------c
c
...
!mpkrb        th(i,j,k)=1.d0-dlog(r)/dlog(r0)
!mpkhc        th(i,j,k)=2.d0*(r-r0)
              th(i,j,k)=0.5d0
...
!mpk2d     +             *th(i,j,k)
         ...
         if(i.eq.0)then 
!mpkdvd           th(i,j,k)=0.5d0
           th(i,j,k)=0.d0
         elseif(i.eq.nr+1)then        
!mpktank
!mpkdvd           th(nr+1,j,k)=dble(k)/dble(np)*1.d0
           th(nr+1,j,k)=1.d0
...
!mpkrb       randno=0.5d0+0.5d0*sin(dble(k)/150.d0*2.d0*pi)
!mpkhc           randno=0.5d0+0.5d0*sin(dble(k)/dble(np))
!mpkhc           th(i,j,k)=th(i,j,k)+(r-r0)/rmax*0.01*((randno*2.d0)-1.d0)
         endif
  10  continue
!mpk      do 11 j=1,nz-1
...
!mpk2d         read(11)nr,nz,np
!mpk2d         read(11)rmax,r0,zmax,zmin
!mpktank        read(11)(((th(i,j,k),i=0,nr+1),j=1,1),k=0,np)
...
c
      do 50 i=0,nr+1
!mpk2d      do 50 j=0,nz+1i
       j=1
!mpktank
       thp(i,j,np+1)=th(i,j,np+1)
!mpktank
      do 50 k=0,np
        Ap(i,j,k)=A(i,j,k)
        Bp(i,j,k)=B(i,j,k)
...
!mpk checking CFL conditions-------------------------------------------
       ...
       if(cflmaxu.gt.1.)then
        print*,'WARNING! CFLU EXCEEDED!'
       endif
c       print*,'CFLMAXU=',cflmaxu

       cflmaxv=0.d0
       do i=1,nr-1
!mpkrb        if(i.eq.0)then
!mpkrb          r=r0
!mpkrb        elseif(i.eq.nr+1)then
!mpkrb          r=rmax
!mpkrb        else
...
!mpktank            write(11)(((th(i,j,k),i=0,nr+1),j=1,1),k=0,np)
            write(11)(((th(i,j,k),i=0,nr+1),j=1,1),k=0,np+1)
!mpk2d        do ken=0,np
!mpk2d          do ien=0,nr+1
...
!mpktank       do k=0,np
       do k=1,np-1
!mpktank         tdmaa(k+1)=1.d0
!mpktank         tdmab(k+1)=4.d0
!mpktank         tdmac(k+1)=1.d0
         tdmaa(k)=1.d0
         tdmab(k)=4.d0
         tdmac(k)=1.d0
         if(k.eq.np)then
...
!mpktank           tdmar(k+1)=3.d0*(C(i,j,k+1)-C(i,j,k-1))*dpi
           tdmar(k)=3.d0*(C(i,j,k+1)-C(i,j,k-1))*dpi
         endif 
       enddo
!mpktank       call ptdma(tdmaa,tdmab,tdmac,tdmas,tdmar,np+1)
       call tdma(tdmaa,tdmab,tdmac,tdmas,tdmar,np-1)
!mpktank       do kk=0,np
       do kk=1,np-1
!mpk2d borrowing B to store Ch
!mpktank          B(i,j,kk)=tdmas(kk+1)
         B(i,j,kk)=tdmas(kk)
       enddo
      enddo
...
!mpktank      do k=0,np
      do k=1,np-1
       do i=1,nr-1
         ...
         if(k.eq.np)ka=1
         if(i.eq.1)then
!mpkbug?          tdmar(i)=1.5d0*(th(2,j,k)+th(2,j,ka)-th(1,j,k)-th(1,j,ka))
!mpkbug?     +      *dri-(-2./3.*(th(0,j,k)+th(0,j,ka)) +0.5d0*(th(1,j,k)+
!mpkbug?     +      th(1,j,ka)) + 1./6.*(th(2,j,k)+th(2,j,ka)))*dri
              tdmar(i)=3.d0*(th(2,j,k)+th(2,j,ka)-th(1,j,k)-th(1,j,ka))
     +      *dri-(-4./3.*(th(0,j,k)+th(0,j,ka)) +1.5d0*(th(1,j,k)+
     +      th(1,j,ka)) - 1./6.*(th(2,j,k)+th(2,j,ka)))*dri
         elseif(i.eq.nr-1)then
!mpkbug?          tdmar(i)=1.5d0*(th(nr,j,k)+th(nr,j,ka)-th(nr-1,j,k)-
!mpkbug?     +      th(nr-1,j,ka))*dri-(-1./6.*(th(nr-1,j,k)+th(nr-1,j,ka))-
!mpkbug?     +      0.5d0*(th(nr,j,k)+th(nr,j,ka))+2./3.*(th(nr+1,j,k)+
!mpkbug?     +      th(nr+1,j,ka) ) )*dri
           tdmar(i)=3.d0*(th(nr,j,k)+th(nr,j,ka)-th(nr-1,j,k)-
     +      th(nr-1,j,ka))*dri-(1./6.*(th(nr-1,j,k)+th(nr-1,j,ka))-
     +      1.5d0*(th(nr,j,k)+th(nr,j,ka))+4./3.*(th(nr+1,j,k)+
     +      th(nr+1,j,ka) ) )*dri
          
         else
!mpkbug?          tdmar(i)=1.5d0*(th(i+1,j,k)+th(i+1,j,ka)-th(i,j,k)
!mpkbug?     +             -th(i,j,ka))*dri
          tdmar(i)=3.d0*(th(i+1,j,k)+th(i+1,j,ka)-th(i,j,k)
     +             -th(i,j,ka))*dri
         endif   
       enddo    
         call tdma(tdmaa,tdmab,tdmac,tdmas,tdmar,nr-1)
...
!mpktank         tdmac(k+1)=1.d0
         ...
         ka=k+1
         if(k.eq.np)ka=1
!mpkbug?          tdmar(k+1)=1.5d0*(th(i+1,j,ka)+th(i,j,ka)-th(i+1,j,k)-
!mpkbug?    +                th(i,j,k))*dpi
!mpktank           tdmar(k+1)=3.d0*(th(i+1,j,ka)+th(i,j,ka)-th(i+1,j,k)-
!mpktank     +                th(i,j,k))*dpi
         tdmar(k)=3.d0*(th(i+1,j,ka)+th(i,j,ka)-th(i+1,j,k)-
     +                th(i,j,k))*dpi
       enddo
!mpktank       call ptdma(tdmaa,tdmab,tdmac,tdmas,tdmar,np+1)
       call tdma(tdmaa,tdmab,tdmac,tdmas,tdmar,np-1)
...
!mpktank        kmm=k-2
        ka=k+1
!mpktank        kaa=k+2
!mpktank not used anyway        if(k.eq.1)kmm=np-1
!mpktank        if(k.eq.np-1)kaa=1
!mpktank        if(k.eq.0)then 
!mpktank         km=np-1
!mpktank         kmm=km-1
!mpktank        endif
!mpktank        if(k.eq.np)then
!mpktank          ka=1
!mpktank          kaa=ka+1
!mpktank        endif
c
!mpkrb        r=r0+dble(i)*dr
        r=1.d0
        ri=1.d0/r
...
!mpktank         tdmac(k+1)=1.d0
         ...
         tdmac(k)=1.d0
!mpktank         if(k.eq.np)then
!mpktank           tdmar(np+1)=3.d0*(C(i,j,1)-C(i,j,np-1))*dpi
!mpktank         elseif(k.eq.0)then
!mpktank           tdmar(1)=3.d0*(C(i,j,1)-C(i,j,np-1))*dpi
!mpktank         else
!mpktank           tdmar(k+1)=3.d0*(C(i,j,k+1)-C(i,j,k-1))*dpi
           tdmar(k)=3.d0*(C(i,j,k+1)-C(i,j,k-1))*dpi
!mpktank         endif 
...
!mpk SERIOUS bug? 20 Feb 2009        RHS1=Crra+Chha*(ri**2)+0.5d0*C(i,j,k)/(Ptl*dt)
...
!mpktank     do k=0,np
      do k=1,np
       do i=1,nr
         ...
         tdmab(i)=4.d0
         tdmac(i)=1.d0
!mpkhc modifications are made to use top (inner cylindrical surface)
!mpkhc wall with insulated b.c.
         if(i.eq.1)then
          tdmab(i)=1.d0
          tdmac(i)=2.d0
!mpkdvd          tdmar(i)=dri*(-8d0*th(0,j,k)+6d0*th(1,j,k)+
!mpkdvd     +             1.9d0*th(2,j,k)+0.1d0*th(4,j,k))
!mpkdvd begin
          tdmar(i)=dri*(0.5333333333d0*th(0,j,k)-3.6d0*th(1,j,k)+
     +         2.9666666667d0*th(2,j,k)+0.1d0*th(4,j,k))
!mpkdvd end
!mpkhc end
         elseif(i.eq.nr)then
          tdmaa(i)=2.d0
...
!mpktank         tdmac(k+1)=1.d0
         ...
         tdmac(k)=1.d0
!mpktank         if(k.eq.np)then
!mpktank           tdmar(np+1)=3.d0*(th(i,j,1)-th(i,j,np-1))*dpi
!mpktank         elseif(k.eq.0)then
!mpktank           tdmar(1)=3.d0*(th(i,j,1)-th(i,j,np-1))*dpi
!mpktank         else
!mpktank           tdmar(k+1)=3.d0*(th(i,j,k+1)-th(i,j,k-1))*dpi
         tdmar(k)=3.d0*(th(i,j,k+1)-th(i,j,k-1))*dpi
!mpktank         endif 
       enddo
!mpktank       call ptdma(tdmaa,tdmab,tdmac,tdmas,tdmar,np+1)
       call tdma(tdmaa,tdmab,tdmac,tdmas,tdmar,np)
!mpktank       do kk=0,np
       do kk=1,np
...
!mpk2d         thr=(-4.d0*th(i-1,j,k)/3.d0+th(i,j,k)+th(i+1,j,k)/3.d0)*dri
          thr=A(i,j,k)
          cnr=-2.5d0*dri**2  !mpkhc bug? +0.5d0*dri*ri
        elseif(i.eq.nr)then
          thrra=(3.2d0*th(i+1,j,k)+2.d0*th(i-1,j,k)-0.2d0*th(i-2,j,k))*
     +          dri**2
...
!mpkhc wall with insulated b.c.
         if(i.eq.1)then
          tdmab(i)=1.d0
          tdmac(i)=2.d0
!mpkdvd           tdmar(i)=dri*(-8d0*th(0,j,k)+6.d0*th(1,j,k)+
!mpkdvd     +             1.9d0*th(2,j,k)+0.1d0*th(4,j,k))
!mpkdvd begin
          tdmar(i)=dri*(0.5333333333d0*th(0,j,k)-3.6d0*th(1,j,k)+
     +         2.9666666667d0*th(2,j,k)+0.1d0*th(4,j,k))
...
!mpk2d         thr=(-4.d0*th(i-1,j,k)/3.d0+th(i,j,k)+th(i+1,j,k)/3.d0)*dri
         thr=A(i,j,k)
          cnr=-2.5d0*dri**2  !mpkhc +0.5d0*dri*ri
        elseif(i.eq.nr)then
          thrra=(3.2d0*th(i+1,j,k)+2.d0*th(i-1,j,k)-0.2d0*th(i-2,j,k))*
     +          dri**2
          thra=(-th(i-1,j,k)/3.d0+4.d0*th(i+1,j,k)/3.d0)*dri
...
!           km=k-1
!           ka=k+1
!mpktank           if(k.eq.0)km=np-1
!mpktank           if(k.eq.np)ka=1 
...
!mpkdvd 2nd order:
!         trans1=trans1+0.5d0*dp*(-2.6666666667d0*th(0,j,k)+
!     +         3.d0*th(1,j,k)-0.3333333333d0*th(2,j,k))*dri+
!     +         0.5d0*dp*(-2.6666666667d0*th(0,j,ka)+
!     +         3.d0*th(1,j,ka)-0.3333333333d0*th(2,j,ka))*dri
...
!mpkdvd 2nd order:
!         trans2=trans2+0.5d0*dp*(2.6666666667d0*th(nr+1,j,k)-
!     +         3.d0*th(nr,j,k)+0.3333333333d0*th(nr-1,j,k))*dri+
!     +         0.5d0*dp*(2.66666666667d0*th(nr+1,j,ka)-
!     +         3.d0*th(nr,j,ka)+0.3333333333d0*th(nr-1,j,ka))*dri
...
!mpktank not needed anyway      do 12 k=0,np
!mpktank        Bn(0,j,k)=-(3.d0*wel(1,j,k)-wel(2,j,k)/3.d0)/dr
!mpktank        Bn(nr,j,k)=-(wel(nr-1,j,k)/3.d0-3.d0*wel(nr,j,k))/dr
!mpktank  12  continue 
!mpk2d      do 13 j=1,nz
...
!mpktank begin
      do i=0,nr
!mpktank it's actually C=del_r(v)-1/rdel_{theta}u
        Cn(i,j,0)=-uel(i,j,1)/dp
        Cn(i,j,np)=uel(i,j,np)/dp
      enddo
!mpktank end 
c
...
!         thn(0,j,k)=1.125d0*thn(1,j,k)-0.125d0*thn(2,j,k)
!mpkdvd          thn(0,j,k)=-30.d0/92.d0*(-3.75d0*thn(1,j,k)+
!mpkdvd     +        5.d0/6.d0*thn(2,j,k)-3.d0/20.d0*thn(3,j,k))
          thn(0,j,k)=0.d0
!mpkhc end
!mpktank        
!mpkdvd         thn(nr+1,j,k)=dble(k)/dble(np)*1.d0
          thn(nr+1,j,k)=1.d0
...
!mpk2d       do 10 j=1,nz
!mpktank        do 10 k=0,np
!mpktank only internal points (see i)
      do 10 k=1,np-1
!mpkrb        r=r0+(dble(i)-0.5d0)*dr
        r=1.d0
...
!mpk2d        endif
!mpktank         urh(i,j,k)=-Ch/r
!c         if(k.eq.1)then
!c          urh=-Cn(i,j,1)/dp
!c         elseif(k.eq.np)then
!c          urh=Cn(i,j,np-1)/dp
!c         else
          urh(i,j,k)=-Ch
!c         endif
   10 continue
...
!mpktank 
          do ii=0,nr
            psi(ii,jj,0)=0.d0
!mpktank this one is outside the defined points; but just to be sure...
            psi(ii,jj,np+1)=0.d0
          enddo
!mpktank end endwalls
...
!mpktank internal points only (see i below)
        do 10 k=1,n3(klev)-1
...
c8aug06            rvali=1.d0/rval
            if(k.eq.1)then
!mpkrb              rval=r0+(dble(i)-0.5d0)*hr(klev)
              rval=1.d0
              rvali(i)=1.d0/rval
              rvalisq(i)=rvali(i)**2.
            if(i.eq.1)then
...
c
          resmax=0.d0
!mpktank          do 20 k=1,n3(klev)
!mpktank internal points only
          do 20 k=1,n3(klev)-1
          do 20 i=1,n1(klev)
c8aug06             rval=r0+(dble(i)-0.5d0)*hr(klev)
c8aug06             rvali=1.d0/rval
...
c8aug06         do 20 k=1,n3(klev)
          ...
          if(k.eq.0)km=n3(klev)-1
          if(k.eq.n3(klev))ka=1
!mpkrb          temp=(psif(i,j,ka)-2.d0*psif(i,j,k)+psif(i,j,km))
!mpkrb     +        *hhpi*rvalisq(i)+psif(i,j,k)*rvalisq(i)
...
!mpk2d        do 10 j=1,n2(klev)
          ...
          ka=k+1
          km=k-1
!mpktank          if(k.eq.n3(klev))ka=1
           if(k.eq.n3(klev))ka=0
          do 11 i=1,n1(klev)-1
...
!mpktank          if(k.eq.n3(klev))ka=1
            if(k.eq.n3(klev))ka=0
!mpkrb          temp=(psif(i+1,j,k)+psif(i-1,j,k))*hhri
!mpkrb     +        +1.5d0*(psif(i+1,j,k)-psif(i-1,j,k))*hri
