...
c     ------------------------------------
c
      parameter(nx=200,ny=200,ng=10000)
c
      dimension x(0:nx),y(0:ny),arr(0:nx,0:ny)
      character*50 label,fname,ans
c
      print*,'data filename'
      read*,fname
      ...
      read(12)xmax,xmin,ymax,ymin
c      read(12)(x(i),i=0,mx)
c      read(12)(y(j),j=0,my)
      read(12)((arr(i,j),i=0,mx),j=0,my)
      close(12)
c
...
c
      ...
      if(ans.eq.'y'.or.ans.eq.'yes')then
        dx=(xmax-xmin)/dble(mx)
        dy=(ymax-ymin)/dble(my)
        do 1 i=0,mx
          x(i)=xmin+dble(i)*dx
    1   continue
        do 2 j=0,my
...
c
      do 210 k=1,nc
          write(6,70)k,cv(k)
c
c mpk added this plotting style
          if(cv(k).lt.0.d0)then
c mpk dashed line for negative cv
c mpk what is the number for dashed line?
             isymb=?
          else
c mpk solid lines for positive cv
             isymb=0
          endif
c
...
c=================================================================
      subroutine grapol(x,y,n,isymb)
      implicit real*8(a-h,o-z)
      real*8 x(n),y(n)
c
      ifail=0
c mpk isymb is the type of line
      call J06BAF(x,y,n,1,isymb,ifail)
c mpk
      if(ifail.ne.0)then
        write(6,100)ifail
