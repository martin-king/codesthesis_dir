...
!mpk Robert filter
        do i=1,nr-1
         do k=0,np
          C(i,j,k)=C(i,j,k)+0.03d0*(Cn(i,j,k)-2.d0*C(i,j,k)+Cp(i,j,k))
         enddo
        enddo
...
!mpk2d         thr=(-4.d0*th(i-1,j,k)/3.d0+th(i,j,k)+th(i+1,j,k)/3.d0)*dri
          thr=A(i,j,k)
!mpk 11aug07 is this a serious bug?          cnr=-2.5d0*dri**2+0.5d0*dr*ri
!mpk 11aug07 fixed          
...
!mpk Robert filter
      do i=1,nr
       do k=0,np
        th(i,j,k)=th(i,j,k)+0.03d0*(thn(i,j,k)-2.d0*th(i,j,k)
     +            +thp(i,j,k))
       enddo
      enddo
c
